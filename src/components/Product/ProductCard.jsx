...
export class ProductCard extends Component {
    render() {
       const {imgUrl, name} = this.props;
        ...
        return (
            <div className={this.props.className + " shadow-md w-9/12 pb-4 my-1 inline-block whitespace-normal"}>
                <div className="w-full bg-center bg-no-repeat" style={{
                    backgroundImage: `url(${imgUrl})`,
                    backgroundSize:"contain",
                    paddingTop: "65%"
                }}/>
                <p className="pl-3 mt-4 font-playfair-display text-sm tracking-wider text-dark-primary-color">{name}</p>
                <p className="px-3 mt-2 font-playfair-display text-xs tracking-wide text-dark-secondary-color break-words">{description}</p>
